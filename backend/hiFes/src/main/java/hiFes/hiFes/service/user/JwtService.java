...
public class JwtService {
    ...
    // access token + refresh token 헤더로 보내기
    ...
    // access token 에서 i 추출
    public Optional<String> extractEmail(String accessToken) {
        try {
            return Optional.ofNullable(JWT.require(Algorithm.HMAC512(secretKey))
                    .build()
                    .verify(accessToken)
                    .getClaim(EMAIL_CLAIM) // claim(email)
    ...
    public void updateUserInfo(String email, String refreshToken) {
        hostUserRepository.findByEmail(email)
                .ifPresentOrElse(
                        user -> user.updateRefreshToken(refreshToken),
                        () -> new Exception("일치하는 회원이 없습니다.")
    ...
    public void updateNormalUserInfo(String email, String refreshToken) {
    ...
    public void updateNormalRefreshToken(String email, String refreshToken) {
        normalUserRepository.findByEmail(email)
                .ifPresentOrElse(
                        user -> user.updateRefreshToken(refreshToken),
                        () -> new Exception("일치하는 회원이 없습니다.")
